...
export default function Accounts({navigation}) {
  ...
  const sendMessage = (userId) => {
    ...
    fetch(`https://api.speech4all.com/admin/accounts/${userId}/message`, {
      method: "PUT",
      headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://api.speech4all.com/admin/account/${id}`, {
        method: "DELETE",
      });
  ...
  const [expandedItemId, setExpandedItemId] = useState(true);
    ...
    const renderItem = ({ item }) => (
      ...
      <Text style={{marginLeft:10,color:'#07b6ff',marginBottom:20}}>{' '}{item.emailID}</Text>
       ...
       {isExpanded === item.id && expandedItemId === item.id && ( 
        <View>
            ...
            <NeumorphicButton
  title={isLoadingConvert ? <ActivityIndicator color="white" size="small" /> : "Send"}
  disabled={isLoadingConvert} onPress={() => sendMessage(item.id)}
/>
</View>
        </View>
       )}
    ...
    return (
      <View style={styles.container}>
       ...
       <NeumorphicHeader title={"Accounts"} 
         right={
         
          <TouchableOpacity style={{backgroundColor:'gray',padding:0,width:80,borderRadius:10,marginLeft:10}}>
          <View style={{backgroundColor:'white',padding:6,width:65,borderRadius:10,marginRight:20,elevation:15}}>
  <NeumorphicText style={{color:'#07b6ff'}}>Logout</NeumorphicText>
          </View>
        </TouchableOpacity>
